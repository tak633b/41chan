...
def generate_agents(
    ...
    for batch_idx in range(0, len(selected), batch_size):
        ...
        entity_list_text = "\n".join(
            ...
            f"役割: {e.get('attributes',{}).get('role','')}"
...
nameは日本人フルネーム必須（例:田中太郎）。概念名禁止。
JSON配列で返せ:
[{{"name":"田中太郎","bio":"プロフ50字","persona":"性格口調立場200字","age":30,"gender":"male","mbti":"INTJ","profession":"職業","interested_topics":["話題"],"tone_style":"worker","stance":{{"position":"賛成","reason":"理由"}},"hidden_agenda":"本音","trigger_topics":["トピック"]}}]"""
    ...
    for idx, entity in enumerate(selected):
        ...
        agent = OracleAgent(
            ...
            stance=_assign_stance(profile_data.get("stance", {}), idx, len(selected)),
            relationships={},
...
def _fallback_profile(
    ...
    """LLM失敗時のフォールバックプロファイル（エンティティベース）"""
    if used_names is None:
        used_names = set()
    attrs = entity.get("attributes", {})
    ...
    if is_individual:
        ...
        age = random.randint(*AGE_RANGES.get(tone, (35, 55)))
        mbti = _select_mbti(tone, stance)

    return {
        "name": name,
        "bio": entity.get("description", entity.get("name", ""))[:200],
        "persona": (
            f"{entity.get('name', '')}は{entity.get('description', '')}。"
            f"立場: {stance}。年齢: {age}歳。性格: {mbti}タイプ。"
        ),
        "age": age,
        "gender": gender,
        "mbti": mbti,
        "profession": attrs.get("role", entity.get("name", "")),
